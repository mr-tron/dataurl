	...
	return []dataURLTest{
		...
		dataURLTest{
			...
			[]item{
				...
				item{itemMediaType, "image"},
				item{itemMediaSep, "/"},
				item{itemMediaSubType, "svg+xml"},
				item{itemParamSemicolon, ";"},
				item{itemUtf8Enc, "utf8"},
				item{itemDataComma, ","},
				item{itemData, "<svg></svg>"},
				item{itemEOF, ""},
			},
			DataURL{
				MediaType{
...
const rep = `^data:(?P<mediatype>\w+/[\w\+\-\.]+)?(?P<parameter>(?:;[\w\-]+="?[\w\-\\<>@,";:%]*"?)+)?(?P<base64>;base64)?(?P<utf8>;utf8)?,(?P<data>.*)$`
